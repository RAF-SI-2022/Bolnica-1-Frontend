...
@Component({
  selector: 'app-nurse-infirmary-workspace',
  templateUrl: './nurse-infirmary-workspace.component.html',
  styleUrls: ['./nurse-infirmary-workspace.component.css']
})
export class NurseInfirmaryWorkspaceComponent implements OnInit {
  ...
  goToWorkspaceEmpty(): void{
    const lbp = 'P0002';
    const url = `/nurse-infirmary-workspace-one/${lbp}`;
    this.router.navigateByUrl(url);
  }

  goToWorkspace(hospitalization: HospitalizationDto): void{
    console.log("Lbp:  " + hospitalization.lbp)
    const url = `/nurse-infirmary-workspace-one/${hospitalization.lbp}`;
